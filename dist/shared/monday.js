...
exports.isMondayConfigured = isMondayConfigured;
exports.getBoardId = getBoardId;
exports.createMondayTask = createMondayTask;
const constants_1 = require("./constants");
const MONDAY_API = 'https://api.monday.com/v2';
// ── CHECK IF MONDAY IS CONFIGURED ─────────────────────────
function isMondayConfigured() {
    return !!process.env.MONDAY_API_KEY && Object.keys(constants_1.MONDAY_BOARDS).length > 0;
}
// ── GET BOARD ID FOR BU ───────────────────────────────────
function getBoardId(bu) {
    // Check direct match
    if (constants_1.MONDAY_BOARDS[bu])
        return constants_1.MONDAY_BOARDS[bu];
    // Fall back to parent BU for iOLX sub-BUs
    if (['iOLX Supply', 'iOL X3 Supply', 'iOLX Demand'].includes(bu)) {
        return constants_1.MONDAY_BOARDS['iOLX Supply'] ?? null;
    }
    return null;
}
// ── CREATE TASK (graceful — logs warning if unconfigured) ──
async function createMondayTask(params) {
    ...
    `;
        ...
        const createItem = res?.data?.create_item;
...
async function getMondayUserId(email) {
